...
public class PixelPerfectSprite : PixelPerfectObject
{
    ...
    Rect spriteRect
    {
        get { return (spriteRenderer.sprite != null) ? spriteRenderer.sprite.rect : new Rect(0, 0, 0, 0); }
    }

    Vector2 spritePivot
    {
        get { return (spriteRenderer.sprite != null) ? spriteRenderer.sprite.pivot : new Vector2(0, 0); }
    }

    float spritePixelPerUnit
    {
        get
        {
            return (spriteRenderer.sprite != null) ? spriteRenderer.sprite.pixelsPerUnit : PixelPerfect.pixelsPerUnit;
        }
    }
    ...
    override protected Vector2 GetPivotToCenter()
    {
        Vector2 normalizedPivot =
            new Vector2(spriteRect.width * 0.5f - spritePivot.x, spriteRect.height * 0.5f - spritePivot.y);
    ...
    {
        return (new Vector2(-(float) spriteRect.width * 0.5f, (float) spriteRect.height * 0.5f)) * pixelScale *
...
public class PixelPerfectSpriteEditor : Editor
{
    ...
    void DrawButtons()
    {
        ...
        runContinously.boolValue = GUILayout.Toggle(runContinously.boolValue, "Run Continously",
            GUI.skin.FindStyle("Button"), GUILayout.Height(24), GUILayout.Width(150));
        useParentTransform.boolValue = GUILayout.Toggle(useParentTransform.boolValue, "Use Parent Transform",
            GUI.skin.FindStyle("Button"), GUILayout.Height(24), GUILayout.Width(150));
        displayGrid.boolValue = GUILayout.Toggle(displayGrid.boolValue, "Show Grid", GUI.skin.FindStyle("Button"),
    ...
    void DrawParallaxField()
    {
        ...
        if (targetCamera != null && targetCamera.normalCamera != null && !targetCamera.normalCamera.orthographic)
        {
            ...
            parallaxLayer.intValue =
                EditorGUILayout.IntSlider(parallaxLayer.intValue, 0, targetCamera.parallaxLayerCount);
            EditorGUILayout.EndHorizontal();
        }
        else
        {
