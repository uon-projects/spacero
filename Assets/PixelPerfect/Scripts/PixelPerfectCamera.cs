...
public class PixelPerfectCamera : MonoBehaviour
{
    ...
    void CalculatePixelPerfectPosition()
    {
        ...
        pixelPerfectPosition = (Vector3) fixedCameraOrigin - (Vector3) VectorToOrigin() +
    ...
    Vector2 VectorToOrigin(int parallaxLayer = 0)
    {
        ...
        {
            return (new Vector2(-(normalCamera.pixelWidth - 1), normalCamera.pixelHeight - 1) + subpixelOffset) * 0.5f *
                PixelPerfect.worldPixelSize / cameraZoom;
        }
        else
        {
            return (new Vector2(-(normalCamera.pixelWidth - 1), normalCamera.pixelHeight - 1) + subpixelOffset) * 0.5f *
                PixelPerfect.worldPixelSize / cameraZoom * GetParallaxLayerScale(parallaxLayer);
        }
    }
    ...
    public void DrawGridFromParallaxIndex(int parallaxLayerIndex, float pixelScale)
    {
        ...
        if (!normalCamera.orthographic)
        {
            gridScaleFactor *= GetParallaxLayerScale(parallaxLayerIndex);
        }

        if (pixelatedPostProcessing)
        {
            gridScaleFactor *= 1f / cameraZoom;
        }

        DrawGrid2D(normalCamera.orthographicSize * normalCamera.aspect * gridScaleFactor,
            normalCamera.orthographicSize * gridScaleFactor, parallaxLayerIndex, pixelScale);
    }
    ...
    public void DrawRectFromParallaxIndex(int parallaxLayerIndex, float pixelScale)
    {
        ...
        DrawRect2D(normalCamera.orthographicSize * normalCamera.aspect * gridScaleFactor,
            normalCamera.orthographicSize * gridScaleFactor, parallaxLayerIndex, pixelScale);
    }
    ...
    void DrawGrid2D(float sizeX, float sizeY, int parallaxLayerIndex, float pixelScale)
    {
        ...
        for (float i = 0;
            i <= 2 * sizeX;
            i += PixelPerfect.worldPixelSize * pixelScale * GetParallaxLayerScale(parallaxLayerIndex))
        {
        ...
        for (float j = 0;
            j <= 2 * sizeY;
            j += PixelPerfect.worldPixelSize * pixelScale * GetParallaxLayerScale(parallaxLayerIndex))
        {
...
public class PixelPerfectCameraEditor : Editor
{
    ...
    Texture pixelPerfectLogo
    {
        get
        {
            ...
            {
                string path =
                    AssetDatabase.GUIDToAssetPath(AssetDatabase.FindAssets("PixelPerfectCamera")[0])
                        .Replace("PixelPerfectCamera.cs", "") + "Internal/Images/pixelperfect_logo.png";
    ...
    Texture pixelattoIcon
    {
        get
        {
            if (pixelattoIcon_ == null)
            {
                string path =
                    AssetDatabase.GUIDToAssetPath(AssetDatabase.FindAssets("PixelPerfectCamera")[0])
                        .Replace("PixelPerfectCamera.cs", "") + "Internal/Images/pixelatto_icon.png";
    ...
    void DrawZoomAndTargetHeight()
    {
        ...
        else
        {
            ...
            {
                targetPixelHeight.intValue = 120;
            }

            if (GUILayout.Button("240"))
            {
                targetPixelHeight.intValue = 240;
            }

            if (GUILayout.Button("480"))
            {
                targetPixelHeight.intValue = 480;
            }

            if (GUILayout.Button("600"))
            {
                targetPixelHeight.intValue = 600;
            }

            if (GUILayout.Button("1080"))
            {
                targetPixelHeight.intValue = 1080;
            }
