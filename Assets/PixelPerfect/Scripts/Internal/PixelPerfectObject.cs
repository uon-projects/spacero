...
public class PixelPerfectObject : MonoBehaviour
{
    ...
    {
        transform.localScale = new Vector3(Mathf.Sign(transform.localScale.x), Mathf.Sign(transform.localScale.y), 1) *
                               GetTransformScaleFactor();
    }

    protected virtual float GetTransformScaleFactor()
    {
        float parallaxScale =
            (pixelPerfectCamera != null) ? pixelPerfectCamera.GetParallaxLayerScale(parallaxLayer) : 1;
