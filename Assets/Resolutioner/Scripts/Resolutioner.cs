...
public class Resolutioner : MonoBehaviour
{
    ...
    float widthFactor
    {
        get { return (float) maxWidth / (float) Screen.width; }
    }

    float heightFactor
    {
        get { return (float) maxHeight / (float) Screen.height; }
    }

    float PixelsToScreen
    {
        get { return 1f / (float) (Screen.height); }
    }

    float ScreenToUnits
    {
        get { return thisCamera.orthographicSize; }
    }
    ...
    void AdjustResolution()
    {
        ...
        {
            if (dirty || checkWidth != maxWidth || checkHeight != maxHeight || checkScreenWidth != Screen.width ||
                checkScreenHeight != Screen.height)
            {
                ...
                thisCamera.rect = new Rect(Mathf.Clamp01(portMargin.x / 2), Mathf.Clamp01(portMargin.y / 2),
                    Mathf.Clamp01(portSize.x), Mathf.Clamp01(portSize.y));
                checkWidth = maxWidth;
    ...
    void DrawResolutionGizmos()
    {
        ...
        for (int i = 0; i < aspectRatios.Count; i++)
        {
            if (aspectRatios[i] != null)
            {
                ...
                Vector3 screenSize = new Vector3(width, height) * gizmoSizeFactor *
                                     (thisCamera.orthographic ? PixelsToUnits : 1);
    ...
    void DrawRect(Vector3 size, Color color)
    {
        ...
        Gizmos.color = color;
        Gizmos.DrawLine(
            transform.position + transform.right * (-size.x) + transform.up * (size.y) +
    ...
    void DrawLabel(Vector3 size, ResolutionerAspect ratio, float offset)
    {
        ...
        handlesStyle.fontSize = (int) (1f /
                                       HandleUtility.GetHandleSize(transform.position + new Vector3(-size.x, size.y) +
                                                                   gizmosDepth * transform.forward));
        float scale = 0.2f *
                      HandleUtility.GetHandleSize(transform.position + new Vector3(-size.x, size.y) +
                                                  gizmosDepth * transform.forward);
        if (orientation == Orientation.Landscape)
        {
            text = " " + ratio.aspectName + " (" + (int) (Screen.height * heightFactor * ratio.ratio) + "x" +
                   Screen.height * heightFactor + ")";
            verticalOffset = (aspectRatios.Count - offset + 1) * scale;
        }
        else
        {
            text = " " + ratio.inverseName + " (" + Screen.height * heightFactor + "x" +
                   (int) (Screen.height * heightFactor * ratio.ratio) + ")";
    ...
    void DrawLabelCamera(Vector3 size)
    {
        ...
        if (nearestAspect != null)
        {
            if (orientation == Orientation.Landscape)
            {
                text = " Current aspect (closest) ≈ " + nearestAspect.aspectName + " (" +
                       (int) (Screen.width * widthFactor) + "x" + Screen.height * heightFactor + ")";
            }
            else
            {
                text = " Current aspect (closest) ≈ " + nearestAspect.inverseName + " (" +
                       Screen.height * heightFactor + "x" + (int) (Screen.width * widthFactor) + ")";
            }
        }

        handlesStyle.normal.textColor = Color.white;
        handlesStyle.fontSize = (int) (1f / HandleUtility.GetHandleSize(transform.position +
                                                                        transform.right * (-size.x) +
                                                                        transform.up * (-size.y) +
                                                                        gizmosDepth * transform.forward));
        Gizmos.DrawLine(
            transform.position + transform.right * (-size.x) + transform.up * (size.y + 1 * scale) +
            gizmosDepth * transform.forward,
            transform.position + transform.right * (-size.x) + transform.up * (size.y) +
            gizmosDepth * transform.forward);
        Handles.Label(
...
public class ResolutionerAspect
{
    ...
    {
        get { return xRatio + ":" + yRatio; }
    }

    public string inverseName
    {
        get { return yRatio + ":" + xRatio; }
    }

    public float ratio
    {
        get { return (float) xRatio / (float) yRatio; }
    }
...
public class ResolutionerAspectDrawer : PropertyDrawer
{
    ...
    public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
    {
        ...
        Rect contentPosition = EditorGUI.PrefixLabel(position,
            new GUIContent("Aspect: " + ((float) xRatio.intValue / (float) yRatio.intValue)));

        float smallWidth = 64f;

        Rect leftRect1 = new Rect(contentPosition.x, contentPosition.y, smallWidth * 0.8f, contentPosition.height);
        Rect separatorRect = new Rect(contentPosition.x + smallWidth * 0.8f + 2, contentPosition.y, 8,
            contentPosition.height);
        Rect leftRect2 = new Rect(contentPosition.x + smallWidth, contentPosition.y, smallWidth * 0.8f,
            contentPosition.height);
        Rect rightRect = new Rect(contentPosition.x + smallWidth * 2, contentPosition.y,
            contentPosition.width - smallWidth * 2, contentPosition.height);
...
{
    SerializedProperty useResolution,
        useDithering,
        useColorFilter,
        usePixelation,
        useSubPixels,
    ...
    SerializedProperty
        postprocessDither,
        postprocessColor,
        postprocessSubPixels,
        postprocessScanlines; //, postprocessPixelate, postprocessGlow;
    ...
    Texture resolutionerLogo
    {
        get
        {
            ...
            {
                string path =
                    AssetDatabase.GUIDToAssetPath(AssetDatabase.FindAssets("Resolutioner")[0])
                        .Replace("Resolutioner.cs", "") + "/Internal/Images/resolutioner_logo.png";
    ...
    Texture pixelattoIcon
    {
        get
        {
            if (pixelattoIcon_ == null)
            {
                string path =
                    AssetDatabase.GUIDToAssetPath(AssetDatabase.FindAssets("Resolutioner")[0])
                        .Replace("Resolutioner.cs", "") + "/Internal/Images/pixelatto_icon.png";
    ...
    void DrawTitle()
    {
        ...
        if (resolutionerLogo != null)
        {
            ...
            if (GUI.Button(
                new Rect(Screen.width - 60, headerRect.y + 4, headerRect.height * 0.875f, headerRect.height * 0.875f),
                new GUIContent(pixelattoIcon, "More Pixelatto Assets")))
            {
    ...
    {
        if (useResolution.boolValue =
            EditorGUILayout.BeginToggleGroup(new GUIContent("Resolution", "Constraint screen resolution"),
                useResolution.boolValue))
        {
    ...
    {
        if (useGizmos.boolValue =
            EditorGUILayout.BeginToggleGroup(new GUIContent("Aspect Gizmos", "Display screen aspect gizmos"),
                useGizmos.boolValue))
        {
            EditorGUI.indentLevel++;
    ...
    void DrawDitherOptions()
    {
        if (useDithering.boolValue =
            EditorGUILayout.BeginToggleGroup(new GUIContent("Dithering", "Discrete color-blending options"),
                useDithering.boolValue))
        {
            EditorGUI.indentLevel++;
    ...
    void DrawColorRampList()
    {
        ...
        foreach (Texture2D ramp in colorRamps)
        {
            ...
            currentRect = new Rect(currentRect.x, currentRect.y + currentRect.height + 2, currentRect.width,
                currentRect.height);
        }
    ...
    void DrawDitherPaletteList()
    {
        ...
        Rect currentRect2 = new Rect(contentRect.x + (contentRect.width) * 0.45f + 2, contentRect.y,
            (contentRect.width) * 0.1f, buttonHeight);
        Rect currentRect3 = new Rect(contentRect.x + (contentRect.width) * 0.55f + 4, contentRect.y,
            (contentRect.width) * 0.45f, buttonHeight);
        EditorGUI.indentLevel = saveIndentLevel;
        foreach (Texture2D palette in palettes)
        {
            ...
            if (processedPalette != null)
            {
                ...
                GUI.DrawTextureWithTexCoords(texRect1.AddMargin(2).SquareInsideRect(), processedPalette,
                    new Rect(0 * texTileWidth, 0, texTileWidth, 0.3333f));
                GUI.DrawTextureWithTexCoords(texRect2.AddMargin(2).SquareInsideRect(), processedPalette,
                    new Rect(7 * texTileWidth, 0, texTileWidth, 0.3333f));
                GUI.DrawTextureWithTexCoords(texRect3.AddMargin(2).SquareInsideRect(), processedPalette,
                    new Rect(15 * texTileWidth, 0, texTileWidth, 0.3333f));
                GUI.DrawTextureWithTexCoords(texRect4.AddMargin(2).SquareInsideRect(), processedPalette,
                    new Rect(23 * texTileWidth, 0, texTileWidth, 0.3333f));
                GUI.DrawTextureWithTexCoords(texRect5.AddMargin(2).SquareInsideRect(), processedPalette,
                    new Rect(31 * texTileWidth, 0, texTileWidth, 0.3333f));
            }
            else
            {
            ...
            currentRect = new Rect(currentRect.x, currentRect.y + currentRect.height + 2, currentRect.width,
                currentRect.height);
            currentRect2 = new Rect(currentRect2.x, currentRect2.y + currentRect2.height + 2, currentRect2.width,
                currentRect2.height);
            currentRect3 = new Rect(currentRect3.x, currentRect3.y + currentRect3.height + 2, currentRect3.width,
                currentRect3.height);
        }
    ...
    void DrawDitherPatternList()
    {
        ...
        Rect currentRect2 = new Rect(contentRect.x + (contentRect.width) * 0.6f, contentRect.y,
            (contentRect.width) * 0.4f, buttonHeight);
        EditorGUI.indentLevel = saveIndentLevel;
        foreach (Texture2D ditherPattern in ditherPatterns)
        {
            ...
            GUI.Label(currentRect2,
                " " + ditherPattern.height + "x" + ditherPattern.height + " (" +
                Mathf.RoundToInt(ditherPattern.width / ditherPattern.height) + " Levels)");
            currentRect = new Rect(currentRect.x, currentRect.y + currentRect.height + 2, currentRect.width,
                currentRect.height);
            currentRect2 = new Rect(currentRect2.x, currentRect2.y + currentRect2.height + 2, currentRect2.width,
                currentRect2.height);
    ...
    {
        if (useColorFilter.boolValue =
            EditorGUILayout.BeginToggleGroup(new GUIContent("Color Filtering", "Override final image colors"),
                useColorFilter.boolValue))
        {
            EditorGUI.indentLevel++;
            ...
            {
                case ((int) Resolutioner.ColorMode.Monochrome):
                    DrawMonochromeOptions();
                    break;
                case ((int) Resolutioner.ColorMode.Grayscale):
                    DrawGrayscaleOptions();
                    break;
                case ((int) Resolutioner.ColorMode.BitLimited):
                    DrawBitLimitOptions();
                    break;
                case ((int) Resolutioner.ColorMode.FixedPalette):
                    DrawFixedPaletteOptions();
                    break;
            }
    ...
    {
        if (useGlow.boolValue =
            EditorGUILayout.BeginToggleGroup(new GUIContent("Glow", "Glow effect options"), useGlow.boolValue))
        {
            EditorGUI.indentLevel++;
    ...
    {
        if (usePixelation.boolValue =
            EditorGUILayout.BeginToggleGroup(new GUIContent("Pixelation", "Set pixels-texels scale"),
                usePixelation.boolValue))
        {
            EditorGUI.indentLevel++;
    ...
    {
        if (useSubPixels.boolValue =
            EditorGUILayout.BeginToggleGroup(new GUIContent("Subpixels", "Subpixel layouts and patterns"),
                useSubPixels.boolValue))
        {
            EditorGUI.indentLevel++;
            ...
            foreach (Texture2D pixelTexture in pixelTextures)
            {
                ...
                currentRect = new Rect(currentRect.x + buttonSize, currentRect.y, currentRect.width,
                    currentRect.height);
            }
    ...
    {
        if (useScanlines.boolValue =
            EditorGUILayout.BeginToggleGroup(new GUIContent("Scanlines", "Overlay scanline pattern"),
                useScanlines.boolValue))
        {
            EditorGUI.indentLevel++;
    ...
    {
        bool isSelected =
            (((PostProcessSubPixels) postprocessSubPixels.objectReferenceValue).renderMaterial.GetTexture("_Pixel") ==
             pixelTexture);
        if (isSelected)
        {
...
public static class ResolutionerExtensionMethods
{
    ...
    {
        return new Rect(inputRect.x + margin, inputRect.y + margin, inputRect.width - 2 * margin,
            inputRect.height - 2 * margin);
    }
