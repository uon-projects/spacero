...
public class PostProcessDither : PostProcessBase
{
    ...
    override protected Material GetRenderMaterial()
    {
        ...
        {
            case Resolutioner.DitherMode.Monochrome:
                result = Resources.Load<Material>("Materials/Dither-Monochrome");
                break;
            case Resolutioner.DitherMode.Indexed:
                result = Resources.Load<Material>("Materials/Dither-Indexed");
                break;
            case Resolutioner.DitherMode.Palette:
                result = Resources.Load<Material>("Materials/Dither-Palette");
                break;
            default:
                result = Resources.Load<Material>("Materials/Dither-Monochrome");
                break;
        ...
        if (renderMaterial != null)
        {
            try
            {
                ...
                if (renderMaterial.HasProperty("_Dithering"))
                {
                    ...
                    renderMaterial.SetFloat("_DitheringLevels",
                        Mathf.FloorToInt(ditherTexture.width / ditherTexture.height));
    ...
    void CheckMaterial()
    {
        if (renderMaterial.name != "Materials/Dither-Monochrome" &&
            resolutioner.ditherMode == Resolutioner.DitherMode.Monochrome)
        {
            renderMaterial = GetRenderMaterial();
        }

        if (renderMaterial.name != "Materials/Dither-Indexed" &&
            resolutioner.ditherMode == Resolutioner.DitherMode.Indexed)
        {
            renderMaterial = GetRenderMaterial();
        }

        if (renderMaterial.name != "Materials/Dither-Palette" &&
            resolutioner.ditherMode == Resolutioner.DitherMode.Palette)
        {
            renderMaterial = GetRenderMaterial();
        }
    }
