...
public static class ColorMap
{
    public static Texture2D Create(Texture2D palette)
    {
        ...
        Texture2D output = new Texture2D(Mathf.RoundToInt(colorResolution * colorResolution),
            Mathf.RoundToInt(colorResolution) * 3, TextureFormat.RGBA32, false);
            ...
            for (int g = 0; g < colorResolution; g++)
            {
                for (int b = 0; b < colorResolution; b++)
                {
                    ...
                    Color secondNearestColor =
                        FindSecondNearestPaletteColor(currentColor, paletteColors, nearestColor, out error2);
                    float ditherWeight = 1 - (error1) / (error1 + error2);
                    SetOutputColor(ref output, r, g, b, nearestColor, secondNearestColor, ditherWeight,
                        colorResolution);
                }
            }
        }
        ...
        System.IO.File.WriteAllBytes(
            Application.dataPath + "/Resolutioner/Resources/ProcessedPalettes/" + palette.name + ".png",
            output.EncodeToPNG());
    ...
    public static Color FindSecondNearestPaletteColor(Color targetColor, Color[] palette, Color nearest,
        out float outError)
    {
        Color result = Color.white;
        float minError = float.MaxValue;
    ...
    public static void SetOutputColor(ref Texture2D outputTexture, int r, int g, int b, Color nearestColor,
        Color secondNearestColor, float ditherWeight, float colorResolution)
    {
