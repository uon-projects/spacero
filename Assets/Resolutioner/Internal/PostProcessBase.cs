...
public abstract class PostProcessBase : MonoBehaviour
{
    public Material renderMaterial
    {
        get
        {
            if (renderMaterial_ == null)
            {
                renderMaterial_ = GetRenderMaterial();
            }

            return renderMaterial_;
        }
        set { renderMaterial_ = value; }
    }
    ...
    protected Resolutioner resolutioner
    {
        get
        {
            if (resolutioner_ == null)
            {
                resolutioner_ = GetComponent<Resolutioner>();
            }
    ...
    void SendImageSizeToShader()
    {
        ...
        {
            if (renderMaterial.HasProperty("_Width"))
            {
                renderMaterial.SetFloat("_Width", Screen.width * Camera.main.rect.width);
            }

            if (renderMaterial.HasProperty("_Height"))
            {
                renderMaterial.SetFloat("_Height", Screen.height * Camera.main.rect.height);
            }
        }
    }
}